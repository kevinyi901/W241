...
Three regression models were developed to evaluate the relationship between message type and a seller’s willingness to reduce price. We tested these models using R v4.3.2 `coeftest()` to test the statistical significance of the coefficients in the linear regression models with robust standard errors (`HC1`) to mitigate potential heteroskedasticity biases in the error estimates.
