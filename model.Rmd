...
# Bar plot of Price Drop
pricedrop_plot <- ggplot(d, aes(x = priceDrop)) +
  geom_bar() +
  labs(
    title = "Bar Plot of Recent Price Drop",
    x = "Price Drop",
    y = "Count"
  )

# Bar plot of Just Listed
listed_plot <- ggplot(d, aes(x = justListed)) +
  geom_bar() +
  labs(
    title = "Bar Plot of Recently Listed",
    x = "Just Listed",
    y = "Count"
  )
...
# vif table
vif_tbl <- tibble(
  Term = names(vif_values_response),
  VIF = as.numeric(vif_values_response)
)
...
vif_tbl <- vif_tbl %>%
  ...
  fmt_number(columns = vars(VIF), decimals = 2)
...
#anova table 
anova_willing_tbl <- broom::tidy(anova_mod)

anova_tbl <- anova_willing_tbl %>%
  gt() %>%
  tab_header(
    title = "ANOVA table for Willingness to Reduce Price"
  ) %>%
  fmt_number(columns = where(is.numeric), decimals =3)

# vif table
vif_willing_tbl <- tibble(
  Term = names(vif_values),
  VIF = as.numeric(vif_values)
)

# Style with gt
vif_willing_tbl <- vif_willing_tbl %>%
  gt() %>%
  tab_header(
    title = "Covariance Balance for Willingness to Reduce Price"
  ) %>%
  fmt_number(columns = vars(VIF), decimals = 2)
```  
